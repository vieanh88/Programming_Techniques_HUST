...
//# khai báo các bien phu tro can thiet
#define MAX_N 100000
#define denta 0.0001
...
inline double sigmoid_fast(double x) {
    ...
    int va_07 = floor((x - start) / denta);

    return sigmoid[va_07] + ((sigmoid[va_07+1] - sigmoid[va_07]) * (x - start - va_07*denta)) / (denta);
